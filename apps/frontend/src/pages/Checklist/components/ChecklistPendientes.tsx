...
import {
  ...
  AlertTriangle, Car, ChevronRight, Filter, Building2,
  Loader2, Lock,
} from "lucide-react";
import {
...
const PAGE_SIZE = 8; // 2 filas × 4 columnas en mobile-first
...
export function ChecklistPendientes({ categories, onOpenWizard }: Props) {
  ...
  // Permiso para abrir el wizard de inspecciones (realizar).
  const canExecute = can("checklist", "inspecciones", "crear");
  // Si el usuario es conductor, su asignación activa define a qué vehículo puede
  // inspeccionar. El wizard usará esto como `presetAssetId` y restringirá la UI.
  ...
  // ── Categorías con pendientes (para sub-tabs) ─────────────────────────────
  ...
  function handleStartInspection(categoryId: string) {
    ...
    if (cat.scopeKind === "pick") {
      // Abrir drawer de selección de activo
      setPickerFor({ categoryId, categoryName: cat.name });
    } else {
      // Wizard directo (los activos ya están derivados)
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex w-max gap-1 md:w-full">
          ...
          {categoriesWithPending.map((c) => (
            <SubTab
              ...
              label={c.name}
              count={c.count}
              icon={c.cadenceKind === "weekly" ? <CalendarClock size={11} /> : c.cadenceKind === "days" ? <AlertTriangle size={11} /> : <Car size={11} />}
            />
          ))}
        </div>
      </div>
      ...
      {/* Drawer de selección de activo (scope pick) */}
      <AnimatePresence>
        {pickerFor && (
          <AssetPickerDrawer
            key="asset-picker"
      ...
      <ChecklistWizard
        ...
        presetDriverId={isConductor && driverState?.hasAssignment ? driverState.assignment.driverId : null}
        onClose={() => {
          setWizardFor(null);
          onOpenWizard(null);
        }}
...
function PendingCategoryCard({
  pending, onStart, canExecute,
}: { pending: PendingCategory; onStart: () => void; canExecute: boolean }) {
  ...
  const urgent = msLeft < 1000 * 60 * 60 * 24; // < 24h
  ...
  return (
      <>
        ...
        >
          <div className="flex flex-col gap-3 border-b border-gray-100 px-4 py-3 dark:border-white/[0.06] sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="truncate text-sm font-bold text-gray-800 dark:text-white">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-cyan-100 px-2 py-0.5 text-[10px] font-bold text-cyan-700 dark:bg-cyan-500/15 dark:text-cyan-300">
                  {pending.pendingItems.length} {pending.pendingItems.length === 1 ? "pendiente" : "pendientes"}
                </span>
              </div>
            ...
            <div className="flex items-center gap-2">
              <span
                ...
                title={`Vence el ${windowEnd.toLocaleString("es-EC")}`}
              >
          ...
          <ul className="divide-y divide-gray-100 dark:divide-white/[0.04]">
            {pending.scopeKind === "pick" ? (
              <li className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                Esta plantilla es de selección libre. Pulsa <strong>Realizar inspección</strong> para elegir el activo.
              </li>
            ) : (
              pending.pendingItems.map((it) => (
                <li key={it.assetId} className="flex items-center justify-between gap-2 px-4 py-2.5">
                  <div className="flex min-w-0 items-center gap-2">
                    <Car size={13} className="shrink-0 text-gray-400" />
  ...
  // ─── AssetPickerDrawer (scope pick) ─────────────────────────────────────────
  // Pide los activos al endpoint de assets con search + paginación 2 filas.
  ...
  function AssetPickerDrawer({
    ...
    /** Si está definido (caso Conductor), el picker solo permite elegir este asset. */
    restrictToAssetId?: string | null;
    /** Datos del asset restringido (lo que ya cargó el padre vía useMyDriverAssignment).
     *  Si viene, evitamos el fetch a /assets que requiere gestion.flotas. */
    ...
    useEffect(() => {
      ...
      // Caso A: picker restringido (Conductor). Usamos el `restrictedAsset` que ya
      //   vino del padre (vía useMyDriverAssignment). No hacemos fetch a /assets
      //   porque ese endpoint requiere `gestion.flotas` y el conductor no lo tiene.
      ...
      // Caso B: flujo normal (supervisor/admin/owner). Fetch al listado de assets.
    ...
    const filtered = useMemo(() => {
      // 1) Si el picker está restringido a un asset (caso Conductor), solo ese.
      let base = assets;
      if (restrictToAssetId) {
        base = assets.filter((a) => String(a.id) === String(restrictToAssetId));
      }
      // 2) Search.
      const q = search.trim().toLowerCase();
      if (!q) return base;
      return base.filter(
        (a) => a.name.toLowerCase().includes(q) || (a.plate?.toLowerCase().includes(q) ?? false)
      );
    }, [assets, search, restrictToAssetId]);

    const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
    const pageItems = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
    // Grid 2 filas en mobile, hasta 4 columnas en desktop.
          ...
          >
            ...
            <div className="shrink-0 border-b border-gray-100 px-5 py-3 dark:border-white/[0.06] sm:px-6">
              ...
              <p className="mt-2 text-[11px] text-gray-500 dark:text-gray-400">
                {loading
                  ? "Cargando activos…"
                  : `${filtered.length} ${filtered.length === 1 ? "activo disponible" : "activos disponibles"}`}
              </p>
            </div>

            {/* Grid de activos (2 filas) */}
                ...
                <div className={`grid gap-3 ${gridCols}`}>
                  {pageItems.map((a) => (
                    ...
                    >
                      ...
                      {a.plate && (
                        <p className="font-mono text-[10px] font-semibold text-cyan-700 dark:text-cyan-300">{a.plate}</p>
                      )}
                      <ChevronRight
                        size={12}
                        className="absolute right-2 top-2 text-gray-300 opacity-0 transition group-hover:translate-x-0.5 group-hover:opacity-100 dark:text-gray-600"
                      />
                    </button>
                  ))}
                </div>
