...
import { withMasterCacheBust } from "@/features/master/helpers";
...
export function MasterSidebar() {
  ...
  const sections = filterSuperadminNavigation(masterNavigation, session?.role ?? "superadmin");
  ...
  return (
    <aside className="app-sidebar hidden min-h-screen w-[286px] shrink-0 border-r border-zinc-800 bg-zinc-950 px-3 py-3 text-white lg:flex lg:flex-col">
      <div className="mb-4 rounded-lg border border-cyan-400/15 bg-[linear-gradient(180deg,rgba(14,25,42,0.98),rgba(9,16,28,0.94))] px-2 py-2">
        <AppLogo
          href={withMasterCacheBust("/master")}
          name="ApliSmart Motors"
          tagline="Panel master"
          theme="dark"
          compact
          mark="full"
          showTagline={false}
            ...
            <div className="space-y-1 px-2 py-2">
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={withMasterCacheBust(item.href)}
      ...
      <div className="mt-3 rounded-lg border border-cyan-400/15 bg-cyan-400/10 px-3.5 py-3">
        ...
        <p className="mt-1 text-xs leading-5 text-zinc-300">
          Empresas, planes, contenido, usuarios y control general del producto.
        </p>
      </div>
    </aside>
  );
