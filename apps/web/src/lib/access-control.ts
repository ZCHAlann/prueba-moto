...
export function getDefaultRouteForRole(role: PlatformRole) {
  return withMasterCacheBust(defaultHomes[role]);
}
...
export function filterOperationalNavigation(
  ...
  return sections
    .map((section) => {
      // Si el usuario no es superadmin, filtramos por sus modulePermissions explícitos
      if (role !== "superadmin") {
        const sectionKey = section.label.toLowerCase().replace(/[\s/]+/g, "_");
        if (sectionKey !== "dashboard" && sectionKey !== "cuenta" && !modulePermissions.includes(sectionKey as PlatformModuleKey)) {
