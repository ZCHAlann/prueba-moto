...
export function MotorEditModal({ motor, onClose }: Props) {
  const { updateMotor } = useMotorAssets();
  ...
  const submit = async () => {
    ...
    await confirmAction({
      ...
      description:         "Se actualizará el registro técnico de este motor.",
      confirmLabel:        "Guardar",
      accent:              "orange",
      successTitle:        "Motor actualizado",
      successDescription:  "Los cambios fueron guardados correctamente.",
      summary: [
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Responsable">
                <input className={inputCls} value={values.responsible} onChange={(e) => set("responsible", e.target.value)} placeholder="Nombre del responsable" />
              </Field>
