...
type Props = { unit: AirConditioningUnit; onClose: () => void };
...
function fromUnit(u: AirConditioningUnit): FormValues {
  return {
    code: u.code,
    name: u.name,
...
export function AcEditModal({ unit, onClose }: Props) {
  ...
  const [existingPhotos, setExistingPhotos] = useState<string[]>(unit.photoUrls);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const uploaded = newFiles.length ? await uploadAcPhotos(newFiles) : [];
      const finalPhotos = [...existingPhotos, ...uploaded];
      const ok = await updateUnit(unit.id, {
        code: values.code,
        name: values.name,
        type: values.type,
      ...
      if (ok) onClose();
      else setGlobalError("No se pudo actualizar la unidad.");
    } catch (err) {
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-white/[0.08] dark:bg-[#0f1623]">
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 bg-white/95 px-6 py-4 backdrop-blur dark:border-white/[0.06] dark:bg-[#0f1623]/95">
            <div>
              <h3 className="text-base font-bold text-gray-800 dark:text-white">Editar unidad A/C</h3>
          ...
          <div className="grid grid-cols-1 gap-4 px-6 py-5 md:grid-cols-2">
            ...
            <Field label="Piso / Área">
              <div className="grid grid-cols-2 gap-2">
                <input className={inputCls} value={values.floor} onChange={(e) => set("floor", e.target.value)} placeholder="Piso" />
                <input className={inputCls} value={values.area} onChange={(e) => set("area", e.target.value)} placeholder="Área" />
              </div>
            </Field>
            <Field label="Marca" required error={errors.brand}>
              <input className={inputCls} value={values.brand} onChange={(e) => set("brand", e.target.value)} />
            </Field>
            <Field label="Modelo">
            ...
            <Field label="Capacidad (BTU)">
              <input className={inputCls} value={values.capacityBtu} onChange={(e) => set("capacityBtu", e.target.value)} />
            </Field>
            <Field label="Voltaje">
              <input className={inputCls} value={values.voltage} onChange={(e) => set("voltage", e.target.value)} />
            </Field>
            ...
            <Field label="Refrigerante">
              <input className={inputCls} value={values.refrigerantType} onChange={(e) => set("refrigerantType", e.target.value)} />
            </Field>
                ...
                <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-3 dark:border-white/[0.08] dark:bg-white/[0.03]">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {existingPhotos.map((src, i) => (
                      <div key={`e-${i}`} className="group relative h-16 w-16 overflow-hidden rounded-lg ring-1 ring-gray-200 dark:ring-white/[0.08]">
                    ...
                    {newPreviews.map((src, i) => (
                      <div key={`n-${i}`} className="group relative h-16 w-16 overflow-hidden rounded-lg ring-1 ring-cyan-300 dark:ring-cyan-500/40">
                        <img src={src} alt="" className="h-full w-full object-cover" />
                        <button
                          type="button"
                          onClick={() => removeNew(i)}
            ...
            <div className="md:col-span-2">
              <Field label="Notas">
                <textarea rows={3} className={textareaCls} value={values.notes} onChange={(e) => set("notes", e.target.value)} />
              </Field>
            </div>
