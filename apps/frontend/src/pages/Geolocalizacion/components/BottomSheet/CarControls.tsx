  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs font-bold leading-tight text-slate-900">{status}</div>
      </div>
    </button>
  );
};
...
export const CarControls = () => {
  ...
  return (
    <div className="space-y-1.5">
      {isBlocked && (
        <div className="flex items-start gap-1.5 rounded-md border border-rose-200 bg-rose-50 p-1.5 text-[9px] text-rose-700">
          <ShieldAlert className="mt-0.5 h-2.5 w-2.5 shrink-0" />
          <div>
            <div className="font-bold">Bloqueado</div>
            <div className="text-rose-600/80">No se puede encender.</div>
          </div>
        </div>
      )}
      ...
      <ControlRow
        ...
        disabled={isBlocked}
        activeBg="border-emerald-200 bg-emerald-50/60"
        activeIcon="bg-emerald-100 text-emerald-600"
        inactiveBg="border-slate-200 bg-white"
        inactiveIcon="bg-slate-100 text-slate-500"
      />
      ...
      <ControlRow
        ...
        isActive={isLocked}
        onClick={toggleLock}
        activeBg="border-rose-200 bg-rose-50/60"
        activeIcon="bg-rose-100 text-rose-600"
        inactiveBg="border-blue-200 bg-blue-50/60"
        inactiveIcon="bg-blue-100 text-blue-600"
      />
    </div>
  );
};