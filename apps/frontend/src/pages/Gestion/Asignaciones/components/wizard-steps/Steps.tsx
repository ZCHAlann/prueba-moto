...
export function Step1ActaInfo({
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <Field label="Acta N.°"      value={data.actaNumber} onChange={(v) => onChange("actaNumber", v)} />
      ...
      <Field label="Lugar"         value={data.actaPlace}  onChange={(v) => onChange("actaPlace", v)}  placeholder="Ciudad / instalación" />
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
          Empresa
        </label>
        <div className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800
          px-3 py-2 text-sm text-gray-500 dark:text-gray-400 select-none">
          {data.companyName || "—"}
        </div>
      </div>
      <Field label="Área / Cuadrilla" value={data.actaArea} onChange={(v) => onChange("actaArea", v)} placeholder="Ej. Cuadrilla Sur" />
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2">
        <Field label="Nombre completo" value={data.driverName}  onChange={(v) => onChange("driverName", v)} />
      </div>
      <Field label="Cédula / DNI"    value={data.driverDni}   onChange={(v) => onChange("driverDni", v)}   placeholder="0000000000" />
      <Field label="Teléfono"        value={data.driverPhone} onChange={(v) => onChange("driverPhone", v)} placeholder="+593 99..." />
      <div className="col-span-2">
        <Field label="Cargo"         value={data.driverRole}  onChange={(v) => onChange("driverRole", v)}  placeholder="Ej. Conductor principal" />
      </div>
    </div>
  );
}
...
export function Step3VehicleData({
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <Field label="Placa"           value={data.vehiclePlate}    onChange={(v) => onChange("vehiclePlate", v)} />
      <Field label="Marca"           value={data.vehicleBrand}    onChange={(v) => onChange("vehicleBrand", v)} />
      <Field label="Modelo"          value={data.vehicleModel}    onChange={(v) => onChange("vehicleModel", v)} />
      <Field label="Color"           value={data.vehicleColor}    onChange={(v) => onChange("vehicleColor", v)} />
      <Field label="Año"             value={data.vehicleYear}     onChange={(v) => onChange("vehicleYear", v)} />
      <Field label="Km al entregar"  value={data.vehicleOdometer} onChange={(v) => onChange("vehicleOdometer", v)} placeholder="Ej. 45230" />
      <Field label="Nivel de combustible" value={data.vehicleFuelLevel} onChange={(v) => onChange("vehicleFuelLevel", v)} placeholder="Ej. 3/4" />
      <Field label="Estado general"  value={data.vehicleCondition} onChange={(v) => onChange("vehicleCondition", v)} placeholder="Ej. Bueno" />
    </div>
  );
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          onChange={(e) => onChange("novedadesText", e.target.value)}
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          onChange={(e) => onChange("accesoriosOtros", e.target.value)}
