...
export function AddInventoryModal({ onClose, onSubmit }: AddInventoryModalProps) {
  ...
  const set = <K extends keyof InventoryForm>(k: K, v: InventoryForm[K]) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="w-full max-w-md overflow-hidden rounded-t-3xl border border-white/[0.08] bg-[#0d1117] shadow-2xl sm:rounded-3xl">
        <div className="h-0.5 bg-teal-500 w-full" />
        <div className="flex items-center justify-between border-b border-white/[0.06] px-6 pb-4 pt-5">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-teal-400">Inventario</p>
            <h2 className="mt-0.5 text-base font-bold text-white">Nuevo repuesto</h2>
          </div>
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-xl text-white/40 hover:bg-white/[0.06] hover:text-white"
        ...
        <div className="max-h-[60vh] overflow-y-auto px-6 py-5 space-y-4">
          ...
          <Field label="Nombre del repuesto" required>
            <input
              className={inputCls}
              placeholder="Filtro de aceite Fleetguard"
              value={form.name}
              onChange={(e) => set("name", e.target.value)}
            />
          </Field>
        ...
        <div className="flex gap-3 border-t border-white/[0.06] bg-white/[0.02] px-6 py-4">
          ...
          <button
            onClick={handleSubmit}
            disabled={saving || !isValid}
            className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-teal-500 py-2.5 text-sm font-bold text-black transition hover:bg-teal-400 active:scale-95 disabled:opacity-40"
          >
