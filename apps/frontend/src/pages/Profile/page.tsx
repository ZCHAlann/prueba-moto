...
function Skeleton({ className }: { className?: string }) {
  return (
    <div className={`animate-pulse rounded-xl bg-gray-100 dark:bg-white/[0.05] ${className}`} />
  );
}

// ─── Modal de cambio de contraseña (stepper 3 pasos) ─────────────────────────
  ...
  async function handleNext() {
    if (step === 1) {
      if (!fields.currentPassword) {
        setErrors({ currentPassword: "Ingresa tu contraseña actual." });
        return;
      }
      setStep(2);
      return;
      ...
      >
        <div className="rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-gray-900 shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 pt-6 pb-4">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/[0.06] transition-colors"
            >
          ...
          <div className="px-6 pb-6">
            <AnimatePresence mode="wait">
              ...
              >
                {step === 1 && (
                  <>
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      Ingresa tu contraseña actual para verificar que eres tú.
                    </p>
                    <Field
                    ...
                    {fields.newPassword.length > 0 && (
                      <div className="space-y-1">
                        <div className="flex gap-1">
                          {[1, 2, 3, 4].map((i) => {
                            const strength = Math.min(Math.floor(fields.newPassword.length / 3), 4);
                            return (
                              <div
                                key={i}
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500">
                          {fields.newPassword.length < 8 ? "Muy corta"
                            : fields.newPassword.length < 12 ? "Aceptable"
                            : fields.newPassword.length < 16 ? "Buena"
                            : "Excelente"}
                        </p>
                      </div>
                    )}
                ...
                {step === 3 && (
                  <>
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      Confirma la nueva contraseña para completar el cambio.
                    </p>
                    <Field
            ...
            <div className="flex items-center justify-between mt-6">
              <button
                type="button"
                onClick={() => step > 1 ? setStep((s) => (s - 1) as PwdStep) : onClose()}
...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (profile.profile.avatarUrl) setAvatarPreview(profile.profile.avatarUrl);
  }, [profile]);
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    await updateProfile({ ...form, avatarUrl: avatarPreview ?? undefined });
  }
        ...
        <div className="grid gap-6 xl:grid-cols-[300px_1fr]">
          ...
          >
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] p-6 flex flex-col items-center gap-4">
              ...
              <div className="relative">
                {/* Anillo exterior — pulso de "señal activa" */}
              ...
              <div className="text-center space-y-1">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500">
                  {profile?.profile.title || session?.roleLabel || "—"}
                </p>
              </div>
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] p-4 space-y-3">
              ...
              {[
                ...
                >
                  ...
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300 text-right max-w-[160px] truncate">
                    {item.value}
                  </span>
                </div>
              ))}
              ...
              <div className="rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <Field
                      label="Usuario"
                      value={form.username ?? ""}
                      onChange={(v) => handleField("username", v)}
                    ...
                    <Field
                      ...
                      value={profile?.email ?? ""}
                      disabled
                      hint="El correo no puede modificarse aquí"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-white/[0.04]">
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    ...
                    {profile?.updatedAt
                      ? new Date(profile.updatedAt).toLocaleDateString("es-EC", {
