...
// ═══════════════════════════════════════════════
//  COCKPIT — getVehicleCockpit  (extendido)
// ═══════════════════════════════════════════════
...
export async function getVehicleCockpit(assetId: string, companyId: string) {
  ...
  const [
    ...
    // Asignación activa (objeto completo — antes solo driverId)
...
export async function getVehicleLocation(assetId: string, companyId: string) {
  ...
  return {
    lat:       row.lastLat  ?? null,
    lng:       row.lastLng  ?? null,
    updatedAt: row.lastGpsAt ?? null,
  };
}
...
export async function toggleEngine(assetId: string, companyId: string) {
  ...
  // Toggle atómico: UPDATE ... SET engine_on = NOT engine_on RETURNING
  const [row] = await db.execute(sql`
    UPDATE company_assets
       SET engine_on = NOT COALESCE(engine_on, false),
           updated_at = NOW()
     WHERE id = ${assetNum}
       AND company_id = ${companyNum}
     RETURNING engine_on
  `) as any;
...
// ═══════════════════════════════════════════════
//  DAILY USAGE — km por hora del día
// ═══════════════════════════════════════════════

export async function getDailyUsage(assetId: string, companyId: string, dateIso: string) {
  const { assetNum, companyNum } = assetIdParams(assetId, companyId);

  // Traemos todas las rutas de ese día, ordenadas por createdAt
  ...
  // Inicializar 24 puntos en 0
  const hours: { hour: number; km: number }[] = Array.from({ length: 24 }, (_, h) => ({ hour: h, km: 0 }));

  // Sumar km por hora (estimado: cada ruta contribuye su distanceKm a la hora de createdAt)
...
// ═══════════════════════════════════════════════
//  STATS — fuel / maintenances / odometer / costs
// ═══════════════════════════════════════════════
...
export async function getStatsMaintenances(assetId: string, companyId: string) {
  ...
  const rows = await db.select({
    scheduledDate: companyMaintenances.scheduledDate,
    createdAt:     companyMaintenances.createdAt,
    status:        companyMaintenances.status,
  }).from(companyMaintenances)
    .where(and(eq(companyMaintenances.assetId, assetNum),
               eq(companyMaintenances.companyId, companyNum),
               gte(companyMaintenances.createdAt, since)));
  ...
  for (const r of rows) {
    ...
    const s = (r.status ?? '') as keyof typeof bucket;
    if (s in bucket) (bucket as any)[s] += 1;
...
export async function getStatsCosts(assetId: string, companyId: string) {
  ...
  const [fuels, maintenances] = await Promise.all([
    ...
    db.select({
      createdAt: companyMaintenances.createdAt,
      cost:      companyMaintenances.cost,
      laborCost: companyMaintenances.laborCost,
      partsCost: companyMaintenances.partsCost,
    }).from(companyMaintenances)
      .where(and(eq(companyMaintenances.assetId, assetNum),
                 eq(companyMaintenances.companyId, companyNum),
                 gte(companyMaintenances.createdAt, since))),
  ...
  for (const m of maintenances) {
    ...
    const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    const total = Number(m.cost ?? 0) + Number(m.laborCost ?? 0) + Number(m.partsCost ?? 0);
    maintMap.set(key, (maintMap.get(key) ?? 0) + total);
  }
...
export async function deleteAssetNote(
  ...
  // Devuelve la nota antes de borrar para que el caller pueda verificar
  // permisos (authorId o admin) — la autorización fina se hace en el route.
